...
interface AuthModalProps {
  ...
  registerUser: (email: string, password: string, username: string, fullName: string) => Promise<{ success: boolean; error?: string; user?: any }>
...
export function AuthModal({
  ...
  useEffect(() => {
    if (showLoginModal) setIsLogin(true);
    if (showRegisterModal) setIsLogin(false);
        ...
        <div className="relative p-6 md:p-8">
          ...
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold mb-2">
              {isLogin ? (t.auth?.signInTitle || "Sign In") : (t.auth?.signUpTitle || "Create Account")}
            </h2>
          ...
          <form 
            onSubmit={async (e) => {
              ...
              if (isLogin) {
                ...
                if (registerEmailRef.current && registerPasswordRef.current && registerUsernameRef.current && registerFullNameRef.current) {
                  const result = await registerUser(
                    registerEmailRef.current.value,
                    registerPasswordRef.current.value,
                    registerUsernameRef.current.value,
                    registerFullNameRef.current.value
                  );
                  if (result.success) {
            ...
            {isLogin ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium pl-1">{t.auth?.username || "Username"}</label>
                    <input
                  ...
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium pl-1">{t.auth?.fullName || "Full Name"}</label>
                    <input
